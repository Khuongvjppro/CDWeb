...
class Order {
  static async createOrder(orderData) {
    ...
    const query = 'INSERT INTO orders (userId, totalAmount, status, shippingAddress, createdAt) VALUES (?, ?, ?, ?, NOW())';
    const result = await pool.execute(query, [userId, totalAmount, status, shippingAddress]);
    return result[0];
  }
  ...
  static async getOrderItems(orderId) {
    const query = 'SELECT oi.*, p.name, p.image FROM order_items oi JOIN products p ON oi.productId = p.id WHERE oi.orderId = ?';
